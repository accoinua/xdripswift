# Збірка xDripSwift 7.0.0 з китайським SIBIONICS GS1 через GitHub Actions
...
4. збирає весь workspace `xdrip.xcworkspace` для фізичного iOS-пристрою без підпису;
5. зберігає unsigned device `.app.zip` і повний Xcode build log як GitHub artifacts.

Unsigned artifact підтверджує, що код, Core Data model і ресурс алгоритму
...
3. Унизу сторінки, у **Artifacts**, завантажте:
   - `xdrip-sibionics-chinese-unsigned-device` — unsigned device app;
