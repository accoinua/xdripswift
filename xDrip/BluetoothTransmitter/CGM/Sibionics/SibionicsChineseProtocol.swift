...
enum SibionicsChineseProtocol {
    ...
    static func dataRequest(nextIndex: Int, macAddress: [UInt8]) -> Data {
        precondition(macAddress.count == 6)
        ...
        // The stock Chinese request carries the sensor BLE address in reverse
        // byte order. CoreBluetooth exposes only an app-scoped UUID, so this
        // address must come from explicit sensor setup data.
        for index in 0..<6 {
